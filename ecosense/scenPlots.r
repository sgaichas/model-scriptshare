...
filename <- "vulbasequants"
outname <- "vul, base scenario"
...
sum_diff <- rowsum(results_end[,5:25], results_end$ScenSpecies)
...
plot((1:(dim(mean_diff2)[1])), seq(min(mean_diff2$CI_5), max(mean_diff2$CI_95), length.out=dim(mean_diff2)[1]), 
      type="n", xlab="", ylab="Biomass range", axes=F)
abline(h=0.0, lty=2)
mtext(paste(outname))
...
par(oma=c(2,2,2,0))
for (i in 1:max(results$ScenSpecies)){
    plotmax<-max(results$CI_95[results$ScenSpecies==i], na.rm=T)
    plotmin<-min(results$CI_5[results$ScenSpecies==i], na.rm=T)
    plot(results$Year[results$ScenSpecies==i], results$CI_50[results$ScenSpecies==i], , ylim=c(plotmin,plotmax), xlab="", ylab="", type="n")
    lines(results$Year[results$ScenSpecies==i], results$CI_95[results$ScenSpecies==i], lty=2)
    lines(results$Year[results$ScenSpecies==i], results$CI_75[results$ScenSpecies==i])
    lines(results$Year[results$ScenSpecies==i], results$CI_50[results$ScenSpecies==i], lwd=2)
    lines(results$Year[results$ScenSpecies==i], results$CI_25[results$ScenSpecies==i])
    lines(results$Year[results$ScenSpecies==i], results$CI_5[results$ScenSpecies==i], lty=2)
